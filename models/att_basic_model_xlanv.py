...
class AttBasicModel(BasicModel):
    def __init__(self, pos=True, tag=True):
        ...
        self.vocab_size = 12982 #cfg.MODEL.VOCAB_SIZE + 1       # include <BOS>/<EOS>
        ...
        #pre-training codes
        self.mlm_head = nn.Linear(cfg.MODEL.BILINEAR.DIM, self.vocab_size)
        self.alm_loss_fct = CrossEntropyLoss(ignore_index=-1)
        self.align_act_dense = nn.Linear(cfg.MODEL.BILINEAR.DIM, 2)
        self.align_con_dense = nn.Linear(cfg.MODEL.BILINEAR.DIM, 2)
    ...
    def preprocess(self, decode = False, **kwargs):
        ...
        if cfg.MODEL.BILINEAR.DIM > 0:
            ...
            if(self.tag):
                ...
                cat_feats = torch.cat([ language_feats, res152_feats, s3d_feats], dim=1)
                cat_mask = torch.cat([ input_mask, res152_mask, s3d_mask], dim = 1)
    ...
    def forward_pretrain(self, **kwargs):
        loss = 0.
        aligned_act = kwargs[cfg.PARAM.ALIGNED_ACT]
        aligned_act = aligned_act.view(-1)
        aligned_con = kwargs[cfg.PARAM.ALIGNED_CON]
        aligned_con = aligned_con.view(-1)
        ...
        align_act_score = self.align_act_dense(language_cross_feats[:, 0, :])
        align_con_score = self.align_con_dense(language_cross_feats[:, 0, :])
        match_loss1 = torch.nn.functional.cross_entropy(align_act_score, aligned_act)
        match_loss2 = torch.nn.functional.cross_entropy(align_con_score, aligned_con)
        acc_match = 0.5 * (accuracy(align_act_score.detach(), aligned_act.detach()) + accuracy(align_con_score.detach(), aligned_con.detach()))
        loss = loss + match_loss1 + match_loss2

        token_labels = kwargs[cfg.PARAM.TOKEN_LABELS_ALL]
        alm_loss, acc = self._calculate_mlm_loss(language_cross_feats, token_labels)
