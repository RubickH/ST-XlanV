...
class Evaler(object):
    def __init__(
        ...
        self.eval_loader = data_loader.load_val(eval_ids, s3d_feats, s3d_logits, res152_feats, res152_logits, test)
    ...
    def __call__(self, model, rname):
        ...
        results = []
        with torch.no_grad():
            for _, (indices, input_ids_all, input_seq, target_seq, s3d_feats, s3d_mask, res152_feats, res152_mask, tag_ids, tag_mask, aligned) in tqdm.tqdm(enumerate(self.eval_loader)):
                ...
                aligned = torch.from_numpy(np.array(aligned)).cuda()
