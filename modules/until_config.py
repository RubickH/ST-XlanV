...
class PretrainedConfig(object):
    ...
    @classmethod
    def get_config(cls, pretrained_model_name, cache_dir, type_vocab_size, state_dict, task_config=None, bert_dir=None):
        if bert_dir is not None:
            config_file = os.path.join(bert_dir, cls.config_name)
            config = cls.from_json_file(config_file)
            config.type_vocab_size = type_vocab_size
            print("load bert config from %s"%config_file)
            weights_path = os.path.join(bert_dir, cls.weights_name)
            if os.path.exists(weights_path):
                state_dict = torch.load(weights_path, map_location='cpu')
                print("load bert state dict from %s"%weights_path)
