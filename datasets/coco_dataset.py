...
class CocoDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.res152_feats_path is not None:
            res152_feats = np.load(os.path.join(self.res152_feats_path, 'video'+str(image_id) + '.npy'))
            res152_feats = np.array(res152_feats).astype('float32')
        else:
            res152_feats = np.zeros((1,1))
            
        if self.s3d_feats_path is not None:
            s3d_feats = np.load(os.path.join(self.s3d_feats_path, 'video'+str(image_id) + '.npy'))
        ...
        s3d_logits = np.load(os.path.join(self.s3d_logits_path, 'video'+str(image_id) + '.npy'))
        res152_logits = np.load(os.path.join(self.res152_logits_path, 'video'+str(image_id) + '.npy'))
        ...
        for i in range(self.seq_per_img):
            ...
            if(self.input_seq is not None):
                prob = random.random()
                if self.pre_train and prob < 0.5:  # yiqing modify here!!!
                    aligned = 0
                    sentence = self.input_seq[self.image_ids[np.random.randint(0, len(self.image_ids))]][np.random.randint(0, self.seq_per_img)]
                else:
                    aligned = 1
                    sentence = self.input_seq[image_id][i]
        ...
        return indices, input_ids_all, input_seq, target_seq, s3d_feats, res152_feats, tag_ids, tag_mask, input_masks_all, token_labels_all, aligned_all, aligned_all
